...
// Temp folder
const TEMP_FOLDER = path.join(__dirname,'app','transaction', 'temp');
if (!fs.existsSync(TEMP_FOLDER)) {
  fs.mkdirSync(TEMP_FOLDER, { recursive: true });
}
...
//   return Array.from(passwords);
// }
// Replace generatePasswordsFromEnv with this new version
function generatePasswordsFromProfile(userProfile) {
  const passwords = new Set();

  // Normalize fields
  const pan = String(userProfile.panCardNumber || "").trim().toUpperCase();
  const dob = String(userProfile.dateOfBirth || "").trim();
  const phone = String(userProfile.phoneNumber || "").trim();
  const firstName = String(userProfile.firstName || "").trim();

  // Raw values
  passwords.add(pan);
  passwords.add(dob.replace(/-/g, ""));
  passwords.add(phone);

  // First 4 letters of name + last 4 digits of phone
  if (firstName && phone) {
    const first4 = firstName.slice(0, 4).toUpperCase();
    const last4 = phone.slice(-4);

    // Base combination
    const base = first4 + last4;

    // Add variations
    passwords.add(base);                    // MAIT2512
    passwords.add(base.toLowerCase());      // mait2512
    passwords.add(first4 + " " + last4);    // MAIT 2512
    passwords.add(first4.toLowerCase() + " " + last4); // mait 2512
    passwords.add(base + " ");              // MAIT2512[space]
  }
  ...
  if (dobParts.length === 3) {
    ...
    const dobDDMMYYYY = `${day}${month}${year}`;
    const dobMMDDYYYY = `${month}${day}${year}`;
    passwords.add(dobDDMMYYYY);
    passwords.add(dobMMDDYYYY);
    passwords.add(dobDDMMYYYY.slice(0, 4));
    passwords.add(dobDDMMYYYY.slice(2));
    passwords.add(dobDDMMYYYY.slice(4));
  }
  ...
  passwords.add(`${nameLower}123`);
  passwords.add(`${nameLower}@123`);
  passwords.add(`${nameLower}${dob.replace(/-/g, "")}`);

  // PAN variations
  passwords.add(pan.slice(5, 9));
  passwords.add(pan.slice(5));
  passwords.add(pan.toLowerCase());
  passwords.add(pan.slice(0, 4) + " " + pan.slice(4));
  passwords.add(pan + " ");

  // Phone number variations
  passwords.add(phone.slice(-4));
  passwords.add(phone.slice(-6));
...
//   }
// }
async function processPDF(filePath, userProfile) {
  try {
    const data = new Uint8Array(fs.readFileSync(filePath));
    ...
    try {
      ...
      if (noPasswordError.message.includes('password')) {
        ...
        const userPasswords = generatePasswordsFromProfile(userProfile);
        console.log(`Generated ${userPasswords.length} possible passwords`);
...
app.post('/process-pdfs', async (req, res) => {
  try {
    ...
    const results = await Promise.all(
      pdfFiles.map(file => processPDF(path.join(TEMP_FOLDER, file), userProfile))
    );
